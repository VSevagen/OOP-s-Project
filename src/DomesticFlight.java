...
public class DomesticFlight extends JFrame
{
	...
	{	
		...
		PPanel1.add(LImg1);
		PPanel1.setBackground(Color.LIGHT_GRAY);
		...
		PPanel2.setBackground(Color.LIGHT_GRAY);
	...
	public static void main(String args[])
	{
		...
		new DomesticFlight(type1, "Domestic Booking Page");
	}
}
...
//read from data
		...
		else
		{
			...
			{
//write into data
				Save2 save2=new Save2(sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate,sUser, iPrice, sTime);
				ObjectOutputStream OOS1 = new ObjectOutputStream(new FileOutputStream("save2"));
				for(i=0;i<iCount;i++)
				{
					Save2 temp1=new Save2(sTempFrom[i], sTempTo[i], sTempClass[i], iTempAdult[i], iTempChildren[i], iTempInfant[i], sTempBookingDate[i],sTempUser[i], iTempPrice[i], sTempTime[i]);
...
//	int iCount;
	public Save2(String sFrom, String sTo, String sClass, Integer iAdult, Integer iChildren, Integer iInfant, String sBookingDate, String sUser, Integer iPrice, String sTime)
	{
		this.sFrom=sFrom;
		this.sTo=sTo;
		this.sClass=sClass;
